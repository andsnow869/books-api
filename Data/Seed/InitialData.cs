//набор стартовых данных (seed) для базы
//Вместо пустой базы заранее подготавливаем список книг (Book), которые будут подставлены в систему при первом запуске.
using Api.Model;

namespace Api.Data.Seed;

public class InitialData //класс InitialData, в котором будет храниться список стартовых книг
{
    public static IEnumerable<Book> Books => new List<Book>
    //static — не нужно создавать объект InitialData, чтобы получить книги. Доступ через InitialData.Books
    //public — свойство доступно из любого места в проекте.
    //IEnumerable<Book> — интерфейс коллекции, по которой можно перебирать элементы в цикле foreach
    //Благодаря static и IEnumerable<Book> получить этот список просто и удобно.
    {
        new Book()
        {
